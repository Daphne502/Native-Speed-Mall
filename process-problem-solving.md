...
在进行 Lighthouse 跑分时，虽然 LCP 有所下降，但 **CLS (Cumulative Layout Shift)** 指标飙升至红色警告区域（0.429），导致总分一度低至55分。

<img src="./assets/CLS-Exception.png" width="70%"/>
...
## Q5：滚动事件性能瓶颈与 Throttle（节流）优化

### Q5问题说明

在实现“返回顶部（Back to Top）”按钮的自动显示与隐藏逻辑时，需要实时监听页面的滚动（`scroll`）事件：

```javascript
// 未优化前的逻辑简述
window.addEventListener('scroll', () => {
// 检查滚动距离并操作 DOM 切换 class
this.checkScrollPosition();
});
```

**现象**：由于原生的 scroll 事件在手指滑动时触发频率极高（通常每秒触发 60-100 次），每动一次手指都会强制浏览器执行一次 JS 逻辑和 DOM 操作（classList.toggle）。在移动端弱网或复杂页面下，这会抢占主线程资源，导致页面滚动出现明显的“掉帧”或卡顿感。

### Q5问题分析

**无效计算冗余**：浏览器窗口的刷新频率通常是 60Hz（即约 16.7ms 刷新一帧）。如果 JS 逻辑的执行频率高于这个数值，多出来的计算结果并不会体现在视觉上，反而造成了 CPU 浪费。所以我们将 `THROTTLE_DELAY` 设置为16ms，意味着每帧只计算一次逻辑，既保证了视觉上的实时响应，又彻底消除了冗余计算。

**主线程阻塞**：scroll 事件属于高频事件，频繁的操作 DOM 会触发重排（Reflow）和重绘（Repaint），如果 JS 占据了太长的时间，渲染进程就会被阻塞，产生“Jank（视觉卡顿）”。

### Q5解决方案

在 Utils 工具类中手动实现一个 Throttle（节流） 函数，将滚动事件的处理频率锁定在浏览器的刷新周期（约 16ms）附近。
...
const Utils = {
    throttle(func, delay) {
        ...
        return (...args) => {
            let now = Date.now();
            // 只有当时间间隔超过设定的延迟（delay）时，才允许执行下一次逻辑
...
// 2. 配合 16ms 的延迟（对应 60FPS），优化 BackToTop 组件
const THROTTLE_DELAY = 16; 
this.scrollHandler = Utils.throttle(() => {
    const isVisible = window.pageYOffset > 300;
    this.element.classList.toggle("visible", isVisible);}, THROTTLE_DELAY);
...
* **计算频率压制**：通过 16ms 的阈值，将每秒上百次的无效触发强制压制在每秒约 60 次左右。

* **渲染性能保障**：确保了 JS 逻辑执行频率与显示器刷新率同步，腾出了主线程空间处理更重要的渲染任务，使“返回顶部”按钮出现时的动画更加丝滑，消除了滚动过程中的微小抖动。
